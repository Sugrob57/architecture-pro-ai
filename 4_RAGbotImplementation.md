...
2. Текст запроса преобразуется в векторное представление (embedding), используя ту же модель (`sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`), что была при формировании индекса.
...
4. Формируется промпт для LLM. Промпт обогащается системными текстами:
    ...
    ФОРМАТ ОТВЕТА:
    1. Шаги, как ты выбрал именно этот напиток.
    1. Описание сорта
    2. Ссылка
    ...
    Подробнее смотри по ссылке: https://t.me/nice_ipa/123
...
6. **Примеры 3–5 успешных диалогов.**
7. **Примеры одного-двух случаев, когда бот будет отвечать: «Я не знаю».**